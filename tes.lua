...
-- ==========================================
-- PROJECT ZEEN TOOLS v9.9 (EXECUTION FIX)
-- ==========================================
-- Update v9.9:
-- [+] CRITICAL FIX: Mengembalikan Temp Script ke Internal Storage
--     (Mengatasi Permission Denied di /sdcard/)
-- [+] ROOT CHECK: Verifikasi akses root saat start
-- [+] STORAGE: Config tetap di /sdcard/Zeen/
-- ==========================================
...
-- ==========================================
-- 1. Path untuk File Eksekusi (WAJIB INTERNAL AGAR BISA DIJALANKAN)
local INTERNAL_DIR = "/data/data/com.termux/files/home"
local TEMP_SCRIPT = INTERNAL_DIR .. "/.zeen_temp.sh"

-- 2. Path untuk Penyimpanan Data User (SDCARD)
...
function exec(cmd)
    ...
    os.execute("chmod 777 " .. TEMP_SCRIPT .. " >/dev/null 2>&1")
    
    -- Jalankan via SU
    local handle = io.popen("su -c '" .. TEMP_SCRIPT .. "' 2>&1")
    if not handle then return "" end
    local result = handle:read("*a")
    handle:close()
    
    -- Jangan hapus file temp terlalu cepat agar tidak race condition
...
function exec_root_mm(cmd)
    ...
    f:close()
    os.execute("chmod 777 " .. TEMP_SCRIPT .. " >/dev/null 2>&1")
    local handle = io.popen("su -mm -c '" .. TEMP_SCRIPT .. "' 2>&1")
    local result = handle:read("*a")
    handle:close()
    return result or ""
end
...
function checkRoot()
    local res = exec("id")
    if res:match("uid=0") or res:match("root") then
        return true
    else
        return false
    end
            ...
            if #files == 0 then print("Tidak ada file .txt di " .. COOKIE_DIR) else
                ...
                if f_idx and files[f_idx] then
                    ...
                    if p_idx and packages[p_idx] then
                        ...
                        if ok then print("SUCCESS!") else print("FAILED: " .. msg) end
                    end
                end
            end
            safe_input("\n[Enter] Kembali.")
...
-- ==========================================
-- AUTO DETECT FIX (UNIVERSAL PARSER)
-- ==========================================
function autoDetectRoblox()
    ...
    if not checkRoot() then
        print("\027[1;31m⚠ PERINGATAN: Script tidak berjalan sebagai Root!\027[0m")
        print("Hasil scan mungkin kosong. Pastikan su diberikan.")
    else
    ...
    if #candidates == 0 then
        ...
        print("\n--- DEBUG INFO ---")
        -- Tampilkan 100 karakter pertama dari output asli untuk debug
        print("Raw Output Sample: " .. raw_output:sub(1, 100))
        safe_input("\nEnter kembali...")
        return
    end
...
function main()
    ...
    while true do
        clearScreen()
        print("ZEEN TOOLS v9.9 (EXECUTION FIX)")
